...
int CTailFrameThread::Run()
{
	...
	while(true)
	{
		...
		{
			for(unsigned int i = 0; i < dwTailFrameCount; i++)	// 依次检查每个仪器IP设置应答帧
			{
		...
		{
			for(unsigned int i = 0; i < dwTailTimeFrameCount; i++)	// 依次检查每个仪器IP设置应答帧
			{
...
{
	// 发送帧
	int iCount = m_TailTimeFrameSocket.SendTo(m_pTailTimeSendData, 128, SendPort, IPBroadcastAddr);
}
...
void CTailFrameThread::TimeDelayCalculation(void)
{
	...
	for (unsigned int i=1; i<uiInstrumentCount;i++)
	{
		...
		m_iTimeHigh = pInstrument->m_uiNetTime - pInstrument->m_uiSystemTime;
		m_iTimeLow = usRecTailTime - pInstrument->m_usTailSendTime - 4112;
		usRecTailTime = pInstrument->m_usTailRecTime;
		// 生成时统报文设置帧
...
{
	int iCount = m_TailTimeFrameSocket.SendTo(m_pTimeDelayData, 128, SendPort, IPBroadcastAddr);
}
