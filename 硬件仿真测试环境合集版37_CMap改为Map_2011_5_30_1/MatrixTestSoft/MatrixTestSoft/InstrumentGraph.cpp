...
// 	m_dcGraph.FillRect(m_rectClient, &m_brushBack) ;
// 	
// 	DrawUnit(iUnitIndex, iLineNum, uiLineDirection, uiType);
...
// 连接线图元尺寸和仪器图元尺寸相等
// uiType							仪器类型
void CInstrumentGraph::DrawUnit(int iUnitIndex, int iLineNum, unsigned int uiLineDirection, unsigned int uiType, bool bIPSetOK)
{
	CPen oPenUnit;
	CPen* pOldPen;
	CDC m_MemDc;			//定义一个DC
