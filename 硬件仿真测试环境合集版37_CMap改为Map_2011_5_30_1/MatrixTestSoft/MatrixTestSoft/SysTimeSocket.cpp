...
void CSysTime::OnReceive(void) 
{
	...
	ret = recvfrom(m_SysTimeSocket, reinterpret_cast<char*>(&udp_buf), sizeof(udp_buf), 0, reinterpret_cast<sockaddr*>(&SenderAddr), &n);
	if(ret == RcvFrameSize) 
	{
...
//		WriteCHToCFile(m_pADCFrameInfo->m_arFileSave, strOutput);
		if (m_pADCFrameInfo->m_FileSave.m_hFile != CFile::hFileNull)
		{
