...
{
	/** 程序配置文件名，包含全部路径*/
	m_strAppIniXMLFile = "..\\parameter\\MatrixLineApp.xml";
	...
	// LCI设备IP地址
	m_strIPLCI = "192.168.0.11";
	//  LCI设备IP地址端口号
	m_uiPortLCI = 0x5010;

	// 和现场仪器通讯的本机IP地址
	m_strIPForInstrument = "192.168.0.11";
	// 和现场仪器通讯的本机IP地址
	m_uiIPForInstrument = CIPList::StringToIP(m_strIPForInstrument);
	// 和内部网络通讯的本机IP地址
...
bool CSiteManage::OnInit()
{
	...
	// 建立仪器测试设置网络对象，基本测试 PortForTestSetBase 37888
	...
	// 建立仪器测试设置网络对象，噪声监测 PortForTestSetNoise 38400
...
void CSiteManage::LoadBufferSizeSetupData()
{
	...
	// 找到数据缓冲区大小设置区
	strKey = "LineServerAppBufferSizeSetup";
	lpDispatch = m_oXMLDOMDocument.getElementsByTagName(strKey);
	oNodeList.AttachDispatch(lpDispatch);
	// 找到入口
	lpDispatch = oNodeList.get_item(0);
