...
{	
	...
	m_oFrameTestSet.m_usPortAim = m_uiPortForTestSet;	// 目标端口号	
...
void CSocketTestSetFrame::ViewSet(int itype)
{
	...
	int istop=0;
	if(itype==1)
		memcpy(&m_oFrameTestSet.m_pFrameData[33], &m_uiIPCount, FramePacketSize2B);
...
void CSocketTestSetFrame::OnSendADCSetCmd(void)
{
	...
	{
	case 1:
		iPos = m_oFrameTestSet.ADCSetFrameHead(BroadCastPort, SendSetCmd, m_uiPortForTestSet);
		m_oFrameTestSet.OnSetTB(iPos, 0, 0, true);
		SendTestSetFrame();
...
*/		break;
	...
	case 17:
		Sleep(ADCOperationSleepTime);
		iPos = m_oFrameTestSet.ADCSetFrameHead(BroadCastPort, SendQueryCmd, m_uiPortForTestSet);
