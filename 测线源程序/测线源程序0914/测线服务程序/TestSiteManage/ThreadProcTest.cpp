...
/*		if(recevicecount==0 || m_pSocketTestSetFrame->gettestcount<1520)
		for(int i=0;i<78;i++){
			if(m_pSocketTestSetFrame->checkrecv[i]==0){
				strDesc.Format("AD checkrecv%d= %d",i,m_pSocketTestSetFrame->checkrecv[i]);
				TRACE2("AD checkrecv%d= %d\r\n",i,m_pSocketTestSetFrame->checkrecv[i]);
			}
		}
*/		
	...
	m_pSiteData->m_oCriticalSectionSystemTimeSite.Lock();
		m_pSiteData->m_bProcTimeDelayON_stop = true;	// 仪器时延处理线程恢复
...
//	Wait(2);		//等待AD命令发送完毕
	...
	/*
	if(m_pSocketTestSetFrame->gettestcount<1560)
		for(int i=0;i<75;i++){
			if(m_pSocketTestSetFrame->checkrecv[i]==0)
				TRACE2("AD checkrecv%d= %d\r\n",i,m_pSocketTestSetFrame->checkrecv[i]);
		}
...
void CThreadProcTest::ProcSendTestSetFrame()
{
	...
	m_pSocketTestSetFrame->m_oFrameTestSet.m_usPortListen =m_pSocketTestSetFrame->m_uiPortForTestSet;
...
*/
	...
	m_pSocketTestSetFrame->m_oFrameTestSet.m_usPortListen =m_pSocketTestSetFrame->m_uiPortForTestSet;
	// 设置数据采集输出的目的端口
	m_pSocketTestSetFrame->m_oFrameTestSet.m_usPortDataOut = m_pSocketTestDataFrame->m_uiPortForTestDataFrame;
...
void CThreadProcTest::GetAndProcTestData()
{
	...
	while(true)
	{
		...
		CString strDesc;
		//strDesc.Format("dIP=%d uiFrameCount=%d ",m_pSocketTestSetFrame->m_oFrameTestSet.m_uiIPAim,uiFrameCount);
		...
		if(m_pTestRequest->m_uiTimeADBegin-0x1000 > uiTimeNow){
			//读空缓冲区
			int uiFrameCount = m_pSocketTestDataFrame->GetFrameCount1();	// by zl
			CString strDesc;
			int tmpidx=0;
		...
		//	byte * tmp=&m_pSiteData->OnReceive_buf[0];
			...
			for(unsigned int i = 0; i < uiFrameCount; i++)
			{
				...
				m_pSiteData->m_oSocketTestDataFrame.Unlock();
				//TRACE3("OnR1 m_iFrameCount=%d,m_irevFrameCount=%d m_iFrameCount=%d\r\n",i,m_pSocketTestDataFrame->m_irevFrameCount,m_pSocketTestDataFrame->m_iFrameCount);
				//TRACE3("OnR2 iCount=%d,iIP=%d idx=%d\r\n",i,m_pSocketTestDataFrame->m_oFrameTestData.m_uiIPSource,m_pSocketTestDataFrame->m_oFrameTestData.m_usDataIndex);
				// 设置测试数据
		...
		//CString strDesc;
		//strDesc.Format("sIP=%d m_usDataIndex=%d",m_pSocketTestDataFrame->m_oFrameTestData.m_uiIPSource,m_pSocketTestDataFrame->m_oFrameTestData.m_usDataIndex);
		//m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcTest", "GetAndProcTestData 2", strDesc);
			}
		...
		m_pSiteData->m_oCriticalSectionSystemTimeSite.Lock();
			uiTimeNow = m_pSiteData->m_uiSystemTimeSite - 300;
			//uiTimeNow = m_pSiteData->m_uiSystemTimeSite -2048-500;
		m_pSiteData->m_oCriticalSectionSystemTimeSite.Unlock();
		...
		for(unsigned int i = 0; i < m_pSocketTestSetFrame->m_uiIPCount; i++)
		{
			// 得到一个单元
			pTestElementData = m_pSocketTestSetFrame->m_pTestElementList->GetTestElementDataByIndex(i);
			if(pTestElementData->m_uigroupendcount==1)
				itmp++;
		}
		m_pSocketTestSetFrame->OnReceiveADCSetReturn(1);
		// 判断：采集结束时间到
		//iAdcount计算收到帧的数量，判断数量是否超过采集点
		int iAdcount=(m_pTestDataBuffer->m_uiFrameIndex*FrameADCount*(m_pTestBaseData->m_uiSamplingRate/1000));
		//if((m_pTestRequest->m_uiTimeCollectEnd  < uiTimeNow) || itmp==m_pSocketTestSetFrame->m_uiIPCount || iAdcount>m_pTestBaseData->m_uiSamplingLength)
		if((m_pTestRequest->m_uiTimeCollectEnd  < uiTimeNow))
		{
			TRACE3("END time=%d itmp=%d m_pSocketTestSetFrame->m_uiIPCount=%d\r\n", uiTimeNow,itmp,saveecount);
...
void CThreadProcTest::TranferView(byte* lpData)
{
	...
	if(FDUNum==0)
		FDUNum=0;
	//取抽样点拼帧
	for(int i=0;i<(FrameADCount72/ViewGain);i++){
		memcpy(ViewBuf+FDUNum*FrameLength+36+ViewOffset[FDUNum]*3+i*3, lpData+36+ViewOffsetFrame[FDUNum]*3+i*ViewGain*3, 3);
		FrameNum[FDUNum]++;
		//接收帧满
		...
		if(FrameNum[FDUNum]%FrameADCount72==0){
			ViewOffsetFrame[FDUNum]=i;
			break;
		}
	}
	ViewOffset[FDUNum]+=(FrameADCount72/ViewGain);
//	if(FDUNum==0)
//		TRACE3("dIP=%d %d m_uiFrameIndex=%d\r\n",FrameNum[FDUNum]/FrameADCount72,ViewOffsetFrame[FDUNum], FrameNum[FDUNum]); 
	//发送帧
	if(FrameNum[FDUNum]%FrameADCount72==0){
		...
		memcpy(&itmp,ViewBuf+FDUNum*FrameLength,sizeof(int));
		// 发送帧
		int iCount = m_pSocketTestSetFrame->SendTo(ViewBuf+FDUNum*FrameLength, FrameLength, m_uiPortLCI, m_strIPLCI);
		ViewOffset[FDUNum]=0;
		memset(ViewBuf+FDUNum*FrameLength+36,0x0,FrameADCount72*3);
		//处理接收帧剩余数据
		for(int i=ViewOffsetFrame[FDUNum]/ViewGain;i<(FrameADCount72/ViewGain);i++){
			memcpy(ViewBuf+FDUNum*FrameLength+36+ViewOffset[FDUNum]*3+i*3, lpData+36+i*ViewGain*3, 3);
			FrameNum[FDUNum]++;
			if(ViewOffsetFrame[FDUNum]+i>FrameADCount72){
				break;
			}
	...
	ViewOffsetFrame[FDUNum]=(ViewOffsetFrame[FDUNum]+FrameADCount72)%ViewGain;
}
...
/*	if(irecevice==6)
		return;
	if(usDataIndex>ADoffset-FrameADCount && pTestElementData->IndexPreview==-1)
		return;
*/
	//设置开始偏移量
	...
	//防止溢出
	if(pTestElementData->m_uiFrameIndex-itmp>3 && itmp-pTestElementData->m_uiFrameIndex>3){
	...
	//m_pTestDataBuffer->m_uiFrameIndex = pTestElementData->m_uiFrameIndex-1;
	int x=usDataIndex;
	...
	//TRACE3("OnR3 iCount=%d,iIP=%d idx=%d\r\n",recevicecount,uiIP,usDataIndex);
	// 设置测试数据
	m_pTestDataBuffer->SetElementTestDataOfOneFrame(lpData, pTestElementData->m_uiIndex, pTestElementData->m_uiFrameIndex);
	strDesc="";
	for(int i=0;i<FrameADCount72;i++)
	{
		CString s1;
		int z=0;
		memcpy(&z, lpData+i*3, 3);
		s1.Format("%06x ",z);
		strDesc=strDesc+s1;
	}
	//m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcTest", "Data", strDesc);
	recevicecount++;
...
//		TRACE1("噪声监测处理帧计数：%d\r\n", m_pTestDataBuffer->m_uiFrameIndexWrite);
		for(unsigned int i = 0; i < m_pSocketTestSetFrame->m_uiIPCount; i++)
		{
			...
			// 计算
			pTestElementData->CalculateTestResult(m_pTestRequest->m_uiType, pData, FrameADCount,m_pTestDataBuffer->m_AVGNoiseValue);
...
void CThreadProcTest::GetTestResultBase()
{
	...
	for(unsigned int i = 0; i < m_pSocketTestSetFrame->m_uiIPCount; i++)
	{
		...
		// 计算
		pTestElementData->CalculateTestResult(m_pTestRequest->m_uiType, pData, m_pTestBaseData->m_uiSamplingPointCount,m_pTestDataBuffer->m_AVGNoiseValue);
		// 判读
...
void CThreadProcTest::StopInstrumentWhichIsCollecting()
{
	...
	while(true)
	{
		...
		CString strDesc;
		strDesc.Format("End uiFrameCount=%d %d ",uiFrameCount);
