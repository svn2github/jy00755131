...
void CInstrumentGraphView::OnShowTestValueInFloatWindow(CInstrument* pInstrument, CPoint pointClient)
{
	...
	strInstrumentType = m_pSiteData->GetBoxTypeStringByID(pInstrument->m_uiInstrumentType);
	if(6 != pParentFrame->m_uiTestType)	// 仪器增益/ 相位。 Gain/Phase 误差
	{		
		strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(pParentFrame->m_uiTestType, 0);
		strTestValue = pInstrument->GetTestResultString(pParentFrame->m_uiTestType, 0, strTestUnit);	
		strValue.Format("%s%d  %s", strInstrumentType, pInstrument->m_uiSN, strTestValue);
	}
	else if(6 == pParentFrame->m_uiTestType)	// 仪器增益/ 相位。 Gain/Phase 误差
	{
		strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(pParentFrame->m_uiTestType, 1);
		strTestValue = pInstrument->GetTestResultString(pParentFrame->m_uiTestType, 1, strTestUnit);	
		CString strValue1 = "";
		strValue1.Format("%s%d  %s", strInstrumentType, pInstrument->m_uiSN, strTestValue);
		strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(pParentFrame->m_uiTestType, 1);
		strTestValue = pInstrument->GetTestResultString(pParentFrame->m_uiTestType, 1, strTestUnit);
		strValue.Format("%s  %s", strValue1, strTestValue);		
	}
