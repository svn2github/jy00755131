...
public: // 属性
	...
	UINT m_uiPortForCmdSendToServer;
...
public: // 方法
	...
	// 得到仪器TestType单位字符串
	CString GetInsarumentTestTypeUnitStringByID(unsigned int uiTestType, int iSubItem);
	// 得到检波器TestType单位字符串
	CString GetSensorTestTypeUnitStringByID(unsigned int uiTestType);
	...
	// 由FTP服务器得到数据文件
	bool GetDataFileFromFTPServer(CFtpConnection* pConnection, CString strFileSource, CString strFileAim);
