...
void CLeftViewBarInstrumentBottom::InitPropList()
{
	...
	pGroupBase->AddSubItem (new CBCGPProp (_T("Auxillary"), oVariant, _T("")));
...
void CLeftViewBarInstrumentBottom::OnShowProperty(unsigned int uiIndex)
{
	...
	// 得到标记文字，依据逻辑值
	oVariant = GetMarkerStringByLogic(pInstrument->m_bAux);
	pSub->SetValue(oVariant);
	...
	strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(4, 0);
	strData = pInstrument->GetTestResultString(4, 0, strTestUnit);
	oVariant = strData;
	pSub->SetValue(oVariant);

	// 仪器畸变测试值
	pSub = pGroupTest->GetSubItem(1);
	strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(5, 0);
	strData = pInstrument->GetTestResultString(5, 0, strTestUnit);
	oVariant = strData;
	pSub->SetValue(oVariant);

	// 仪器串音测试值
	pSub = pGroupTest->GetSubItem(2);
	strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(7, 0);
	strData = pInstrument->GetTestResultString(7, 0, strTestUnit);
	oVariant = strData;
	pSub->SetValue(oVariant);

	// 仪器增益测试值
	pSub = pGroupTest->GetSubItem(3);
	strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(6, 1);
	strData = pInstrument->GetTestResultString(6, 1, strTestUnit);
	oVariant = strData;
	pSub->SetValue(oVariant);

	// 仪器相位测试值
	pSub = pGroupTest->GetSubItem(4);
	strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(6, 2);
	strData = pInstrument->GetTestResultString(6, 2, strTestUnit);
	oVariant = strData;
	pSub->SetValue(oVariant);

	// 仪器共模测试值
	pSub = pGroupTest->GetSubItem(5);
	strTestUnit = m_pSiteData->GetInsarumentTestTypeUnitStringByID(8, 0);
	strData = pInstrument->GetTestResultString(8, 0, strTestUnit);
	oVariant = strData;
	pSub->SetValue(oVariant);
...
*/
COleVariant CLeftViewBarInstrumentBottom::GetMarkerStringByLogic(bool bValue)
{
	COleVariant oVariant;
