...
void CMainFrame::OnProcInterFaceCommand(CFrameInterface* pFrameInterface)
{
	...
	case 200 + 22:	// 处理测试数据	200 + 22 测试数据-检波器测试-检波器噪声监测
		...
		break;
	case 200 + 45:	// 处理测试数据	200 + 45 测试数据-检波器测试-新加检波器的初始测试
		ProcInterFaceDataForOnFieldTest(pFrameInterface);
		break;
	}
}
...
/**
* 新加检波器的初始测试后，更新检波器列表视图
* @return void
*/
void CMainFrame::OnRefreshSensorViewForOnFieldTest()
{
	CBCGPVisualStudioGUIDemoApp* pApp = (CBCGPVisualStudioGUIDemoApp*)AfxGetApp();
	...
	while(NULL != posDoc)
	{
		...
		{
			CSensorListView* pSensorListView = (CSensorListView*)pDocument->GetNextView(posView);
			pSensorListView->OnRefreshForNewTestValue(45);
		}
	}
}
...
void CMainFrame::ProcInterFaceDataForInstrumentTest(CFrameInterface* pFrameInterface, int iTestType)
{
	...
	byte byTestCheck = 0;
	float fTestValue1 = 0.0;
	byte byTestCheck1 = 0;
	byte byTestCheck2 = 0;
	byte byTestCheckAll = 0;
...
* @return void
*/
void CMainFrame::ProcInterFaceDataForOnFieldTest(CFrameInterface* pFrameInterface)
{
	...
	{
		iPos = 0;
		memcpy(&uiIP, &pTestData[i * 6], 4);
		iPos = iPos + 4;
		memcpy(&byTestOnField, &pTestData[i * 6 + iPos], 1);
		iPos = iPos + 1;
		memcpy(&byOnFieldCheck, &pTestData[i * 6 + iPos], 1);

		if(TRUE == m_oSiteData.m_oChannelList.m_oChannelMap.Lookup(uiIP, pSensor))
		{
			m_oSiteData.m_olsIPSensor.AddTail(uiIP);
			pSensor->m_byTestOnField = byTestOnField;
			pSensor->m_byOnFieldCheck = byOnFieldCheck;
		}
	}
	// 新加检波器的初始测试后，更新检波器列表视图
	OnRefreshSensorViewForOnFieldTest();
}
