...
public: //属性
	...
	byte m_byPhaseCheck;
	/** 仪器增益和相位测试结果 0：未测试；1：测试合格；2：测试不合格；3：无信号*/
...
public: // 方法
	// 得到测试结果字符串
	CString GetTestResultString(unsigned int uiType, int iSubItem, CString strUnit);
	// 得到测试结果字符串
	CString GetTestResultString(byte byCheck, float m_fValue, CString strUnit);
	// 得到测试结果字符串
	CString GetTestResultString(unsigned int uiType, int iSubItem);
	// 得到测试结果字符串
	CString GetTestResultString(byte byCheck, float m_fValue);
	// 得到测试结果字符串
	CString GetTestResultString(byte byCheck);
	// 得到测试结果
	byte GetTestResultCheck(unsigned int uiType);
};
