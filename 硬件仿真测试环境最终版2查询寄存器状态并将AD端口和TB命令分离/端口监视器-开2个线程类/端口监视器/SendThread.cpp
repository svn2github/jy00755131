...
, m_csGetStatusReturnShow(_T(""))
, m_uiRecFrameNum(0)
, m_uiADCDataRecNum(0)
, m_usADCLastDataPoint(0)
, m_uiGetStatusReturnNum(0)
{
		...
		if(0 != dwFrameCount)	
		{
			for(unsigned int i = 0; i < dwFrameCount; i++)	// 依次检查每个仪器IP设置应答帧
			{
				...
				if (icount == 256)
				{
					...
					else if (uiPort == GetStatusPort)
					{
						// 显示IP
						strTemp.Format(_T("%02x"),udp_buf[udp_count][16]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][17]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][18]);
						m_csGetStatusReturnShow += strTemp;
						...
						// 显示TC时刻低位
						strTemp.Format(_T("%02x"),udp_buf[udp_count][49]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][50]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][51]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][52]);
						m_csGetStatusReturnShow += strTemp;
						m_csGetStatusReturnShow += _T(" ");

						// 显示TE时刻高位
						strTemp.Format(_T("%02x"),udp_buf[udp_count][54]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][55]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][56]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][57]);
						m_csGetStatusReturnShow += strTemp;
						m_csGetStatusReturnShow += _T(" ");

						// 显示TE时刻低位
						strTemp.Format(_T("%02x"),udp_buf[udp_count][59]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][60]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][61]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][62]);
						m_csGetStatusReturnShow += strTemp;
						m_csGetStatusReturnShow += _T(" ");

						// 显示3D命令
						strTemp.Format(_T("%02x"),udp_buf[udp_count][64]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][65]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][66]);
						m_csGetStatusReturnShow += strTemp;
						strTemp.Format(_T("%02x"),udp_buf[udp_count][67]);
						m_csGetStatusReturnShow += strTemp;
						m_csGetStatusReturnShow += _T("\r\n");

						pDlg->GetDlgItem(IDC_EDIT_GETSTATUSRETURNSHOW)->SetWindowText(m_csGetStatusReturnShow);
						m_uiGetStatusReturnNum++;
						strTemp.Format(_T("%d"),m_uiGetStatusReturnNum);
						pDlg->GetDlgItem(IDC_STATIC_GETSTATUSRETURNNUM)->SetWindowText(strTemp);
...
void CSendThread::OnReset()
{
	...
	m_csGetStatusReturnShow = _T("");
	pDlg->GetDlgItem(IDC_EDIT_GETSTATUSRETURNSHOW)->SetWindowText(_T(""));
