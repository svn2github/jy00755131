...
public:
	...
	// 查询状态Socket
	CSocket m_GetStatusSocket;
	// 状态查询帧
	byte m_pGetStatusSendData[128];
	// 设置AD采样数据返回端口和IP后确定有效数据区
	BOOL m_bValidADCData;
public:
	// 创建图形显示模板
	...
	// 发送采集站本地时间查询帧
	void SendCollectSysTimeFrameToSocket();
...
public:
	...
	// 发送数据采集命令帧
	void OnSendCmdFrame(UINT32 uiTestNb);
	afx_msg void OnClose();
	// 创建获取采集站本地时间端口
	void OnCreateCollectSysTimeBroadcastSocket(void);
	// 生成状态查询帧
	void MakeGetStatusFrameData(unsigned int uiIpAim);
	// 发送状态查询帧
	void SendStatusFrameData(void);
};
