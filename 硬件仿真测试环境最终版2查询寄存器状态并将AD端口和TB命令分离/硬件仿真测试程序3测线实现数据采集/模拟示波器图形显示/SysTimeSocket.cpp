...
void CSysTimeSocket::OnReceive(int nErrorCode) 
{
	...
	else if(ret != 0xffffffff) 
	{
		...
		for (int i=0; i<GraphViewNum; i++)
		{
			...
			{
				m_uiIPAim	= 111 - 10*i;
				pHardwareTestDlg->m_bValidADCData = FALSE;
				MakeSetPortFrameData(m_uiIPAim);
				SendFrameData();
			}
		}
		Sleep(500);
		for (int i=0; i<GraphViewNum; i++)
		{
			...
			{
				m_uiIPAim	= 111 - 10*i;
				pHardwareTestDlg->m_bValidADCData = TRUE;
...
// 设置ADC采样数据返回端口和IP地址
void CSysTimeSocket::MakeSetPortFrameData(unsigned int uiIPAim)
{
	unsigned int uiIPSource =	0;
	unsigned int usPortAim	=	0;
	unsigned int usCommand	=	0;
