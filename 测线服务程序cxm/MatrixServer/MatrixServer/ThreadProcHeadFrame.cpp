...
void CThreadProcHeadFrame::ProcHeadFrameOne(CFrameHead* pFrameHead)
{
	...
	// 判断仪器SN是否在SN索引表中
	if(TRUE == m_pSiteData->m_oInstrumentList.IfIndexExistInSNMap(pFrameHead->m_uiSN))
	{
		// 更新路由对象的路由时间，LCI的路由IP地址为0
		m_pSiteData->UpdateRoutTime(pFrameHead->m_uiRoutIP);
		// 在索引表中则找到该仪器,得到该仪器指针
		if (TRUE == m_pSiteData->m_oInstrumentList.GetInstrumentFromSNMap(pFrameHead->m_uiSN, pInstrument))
		{
			// 设置新仪器的首包时刻
			pInstrument->m_uiTimeHeadFrame = pFrameHead->m_uiTimeHeadFrame;
			// 更新仪器的位置信息
			m_pSiteData->SetInstrumentLocation(pInstrument);
		}
	}
	else
	{
		...
		{
			SetCrossRout(pInstrument, DirectionTop);
			SetCrossRout(pInstrument, DirectionDown);
...
// 	if(pFrameHead->m_uiSN==4353)
// 		pFrameHead->m_uiSN=pFrameHead->m_uiSN;
// 		// 判断仪器是否有路由地址；没有路由地址的仪器为主交叉站
// 	if(0 == pFrameHead->m_uiRoutIP )
// 	{
// 		if(1 != pFrameHead->m_uiInstrumentType)
// 			return;
// 		// 新仪器指针为空
// 		CInstrument* pInstrument = NULL;
// 		// 得到新仪器
...
// 	else
// 	{
// 		if(m_pSiteData->HeadCount<5 && 1 != pFrameHead->m_uiInstrumentType){
...
// 		// 增加仪器标志设置为真
// 		m_bInstrumentAdd = true;
// 		TRACE3("SN=%d m_uiRoutIP=%d m_uiRoutDirection=%x\r\n",pFrameHead->m_uiSN, pFrameHead->m_uiRoutIP,pRout->m_uiRoutDirection); 
// 
// 		// 判断有路由地址，来自交叉站上方
...
// 			//清除上个仪器的尾包计数
// 			//pInstrument->m_pInstrumentRight->m_uiTailFrameCount=0;
// 
...
// 			//清除上个仪器的尾包计数
// 			//pInstrument->m_pInstrumentLeft->m_uiTailFrameCount=0;
// 			// 链接仪器，路由尾仪器右指针指向新仪器
...
void CThreadProcHeadFrame::SetCrossRout(CInstrument* pInstrument, unsigned int uiRoutDirection)
{
	...
	pRout->UpdateRoutTime();
	// 路由对象加入路由索引表
	m_pSiteData->m_oRoutList.AddRout(pRout->m_uiRoutIP, pRout);
}
// 
// /**
// * 增加一个需要设置IP地址的仪器
