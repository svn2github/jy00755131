...
// 输出错误帧的帧内容
MATRIXSERVERDLL_API string OutPutErrorFrameInfo(char* pFrameData, int iSize)
{
	string str = "";
	CString cstr = _T("");
	if (pFrameData == NULL)
...
{
	if ((pLogOutPut == NULL)
		|| ((OutPutSelect == 1) && (pLogOutPut->m_byLogFileType != OptLogType)))
	{
		return -1;
	}
	CString strPath = _T("");
	...
	err = fopen_s(&pLogOutPut->m_pFile, ConvertCStrToStr(strPath).c_str(), 
		"wt+, ccs=UNICODE");
	...
	unsigned int uiErrorCode = 0)
{
	if ((pLogOutPut == NULL)
		|| ((OutPutSelect == 1) 
		&& ((byErrorType != ErrorType) || (pLogOutPut->m_byLogFileType != OptLogType))))
	{
		return;
	}
...
{
	if ((pConstVar == NULL) || (pFrameData == NULL))
	{
...
{
	if ((pConstVar == NULL) || (pCommand == NULL) || (pFrameData == NULL))
	{
	...
	{
		AddMsgToLogOutPutList(pConstVar->m_pLogOutPut, "ParseInstrumentFrame", 
			OutPutErrorFrameInfo(pFrameData, pConstVar->m_iRcvFrameSize), ErrorType, IDS_ERR_CHECKFRAMEHEAD);
		return false;
	}
	...
	while(true)
	{
		...
		{
			memcpy(&pCommand->m_byFDUErrorCodeDataCount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pCommand->m_byFDUErrorCodeCmdCount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pCommand->m_byTimeStatus, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pCommand->m_byCtrlStatus, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
		}
		...
		{
			memcpy(&pCommand->m_byLAUXRoutOpenQuery, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize4B;
		}
		else if (byCommandWord == pConstVar->m_byCmdLAUXRoutOpenSet)
		{
			memcpy(&pCommand->m_byLAUXRoutOpenSet, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize4B;
		}
		...
		{
			memcpy(&pCommand->m_pcADCSet[pCommand->m_iADCSetNum], &pFrameData[iPos], iFramePacketSize4B);
			iPos += iFramePacketSize4B;
		...
		{
			memcpy(&pCommand->m_byLAUXErrorCodeDataLineACount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pCommand->m_byLAUXErrorCodeDataLineBCount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pCommand->m_byLAUXErrorCodeDataLAUXLineACount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pCommand->m_byLAUXErrorCodeDataLAUXLineBCount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize1B;
		}
		else if (byCommandWord == pConstVar->m_byCmdLAUXErrorCode2)
		{
			memcpy(&pCommand->m_byLAUXErrorCodeCmdCount, &pFrameData[iPos], iFramePacketSize1B);
			iPos += iFramePacketSize4B;
		}
	...
	unsigned short usCommandWordNum = 0)
{
	...
	for (unsigned short i=0; i<usCommandWordNum; i++)
	{
		...
		{
			memcpy(&pFrameData[iPos], &pCommand->m_byFDUErrorCodeDataCount, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pFrameData[iPos], &pCommand->m_byFDUErrorCodeCmdCount, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pFrameData[iPos], &pCommand->m_byTimeStatus, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pFrameData[iPos], &pCommand->m_byCtrlStatus, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
		}
		...
		{
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXRoutOpenQuery, iFramePacketSize1B);
			iPos += iFramePacketSize4B;
		}
		else if (pCommandWord[i] == pConstVar->m_byCmdLAUXRoutOpenSet)
		{
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXRoutOpenSet, iFramePacketSize1B);
			iPos += iFramePacketSize4B;
		}
		...
		else if (pCommandWord[i] == pConstVar->m_byCmdADCSet)
		{
			...
			{
				memcpy(&pFrameData[iPos], &pCommand->m_pcADCSet[0], iFramePacketSize4B);
				iPos += iFramePacketSize4B;
			}
			else
			{
				...
				for (int j = iFramePacketSize4B; j < pCommand->m_iADCSetNum; j += iFramePacketSize4B)
				{
					...
					iPos += iFrameCmdSize1B;
					memcpy(&pFrameData[iPos], &pCommand->m_pcADCSet[j], iFramePacketSize4B);
					iPos += iFramePacketSize4B;
				}
			}
			memcpy(&pFrameData[iPos], &pCommand->m_pcADCSet[pCommand->m_iADCSetNum], iFramePacketSize4B);
			iPos += iFramePacketSize4B;
			pCommand->m_iADCSetNum += iFramePacketSize4B;
		}
		...
		{
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXErrorCodeDataLineACount, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXErrorCodeDataLineBCount, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXErrorCodeDataLAUXLineACount, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXErrorCodeDataLAUXLineBCount, iFramePacketSize1B);
			iPos += iFramePacketSize1B;
		}
		else if (pCommandWord[i] == pConstVar->m_byCmdLAUXErrorCode2)
		{
			memcpy(&pFrameData[iPos], &pCommand->m_byLAUXErrorCodeCmdCount, iFramePacketSize1B);
			iPos += iFramePacketSize4B;
		}
		...
		{
			memset(&pFrameData[iPos], pConstVar->m_bySndFrameBufInit, iFramePacketSize4B);
			iPos += iFramePacketSize4B;
		}
	}
	pFrameData[iPos] = pConstVar->m_byCmdEnd;
	...
	int iRcvFrameSize, m_oLogOutPutStruct* pLogOutPut = NULL)
{
	...
	{
		AddMsgToLogOutPutList(pLogOutPut, "GetFrameData", OutPutErrorFrameInfo(pFrameData, iCount), 
	...
	unsigned short usPort, unsigned int uiIP, m_oLogOutPutStruct* pLogOutPut = NULL)
{
	...
	{
		AddMsgToLogOutPutList(pLogOutPut, "SendFrame", OutPutErrorFrameInfo(pFrameData, iCount), 
...
MatrixServerDll_API void LoadIniFile(m_oConstVarStruct* pConstVar)
{
	...
	try
	{
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCStopSample, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCOpenTBPowerLow, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCOpenTBPowerHigh, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCOpenSwitchTBLow, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCOpenSwitchTBHigh, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCRegisterRead, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCRegisterWrite, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCTBSwitchOpen, 
		...
		GetPrivateProfileString(strSection,strSectionKey,NULL,strBuff,sizeof(strBuff) / 2,strFilePath);
		strValue = strBuff;
		ParseCStringToArray(&pConstVar->m_pSetADCReadContinuous, 
...
{
	if ((pConstVar == NULL) || (pHeartBeatFrame == NULL) || (pCommInfo == NULL))
	{
	...
	memset(pHeartBeatFrame->m_pcSndFrameData, pConstVar->m_bySndFrameBufInit, 
	...
	memset(pHeartBeatFrame->m_pcCommandWord, pConstVar->m_bySndFrameBufInit, 
...
MatrixServerDll_API void OnInitInstrumentHeadFrame(m_oHeadFrameStruct* pHeadFrame, 
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pHeadFrame == NULL) || (pCommInfo == NULL))
	{
...
MatrixServerDll_API void OnInitInstrumentIPSetFrame(m_oIPSetFrameStruct* pIPSetFrame,
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pIPSetFrame == NULL) || (pCommInfo == NULL))
	{
	...
	memset(pIPSetFrame->m_pcCommandWord, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iCommandWordMaxNum);
	// IP地址设置命令字个数
...
MatrixServerDll_API void OnInitInstrumentTailFrame(m_oTailFrameStruct* pTailFrame, 
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pTailFrame == NULL) || (pCommInfo == NULL))
	{
...
MatrixServerDll_API void OnInitInstrumentTailTimeFrame(m_oTailTimeFrameStruct* pTailTimeFrame,
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pTailTimeFrame == NULL) || (pCommInfo == NULL))
	{
	...
	memset(pTailTimeFrame->m_pcSndFrameData, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iSndFrameSize);
	// 清空尾包时刻查询命令字集合
	...
	memset(pTailTimeFrame->m_pcCommandWord, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iCommandWordMaxNum);
	// 尾包时刻查询命令字个数
	...
	memset(pTailTimeFrame->m_pcRcvFrameData, pConstVar->m_bySndFrameBufInit, 
...
MatrixServerDll_API void OnInitInstrumentTimeDelayFrame(m_oTimeDelayFrameStruct* pTimeDelayFrame,
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pTimeDelayFrame == NULL) || (pCommInfo == NULL))
	{
	...
	memset(pTimeDelayFrame->m_pcSndFrameData, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iSndFrameSize);
	// 清空时统设置命令字集合
	...
	memset(pTimeDelayFrame->m_pcCommandWord, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iCommandWordMaxNum);
	// 时统设置命令字个数
	...
	memset(pTimeDelayFrame->m_pcRcvFrameData, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iRcvFrameSize);
...
MatrixServerDll_API void OnInitInstrumentADCSetFrame(m_oADCSetFrameStruct* pADCSetFrame,
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pADCSetFrame == NULL) || (pCommInfo == NULL))
	{
...
MatrixServerDll_API void OnInitInstrumentErrorCodeFrame(m_oErrorCodeFrameStruct* pErrorCodeFrame,
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pErrorCodeFrame == NULL) || (pCommInfo == NULL))
	{
	...
	memset(pErrorCodeFrame->m_pcSndFrameData, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iSndFrameSize);
	// 清空误码查询命令字集合
	...
	memset(pErrorCodeFrame->m_pcCommandWord, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iCommandWordMaxNum);
	// 误码查询命令字个数
	...
	memset(pErrorCodeFrame->m_pcRcvFrameData, pConstVar->m_bySndFrameBufInit, 
		pConstVar->m_iRcvFrameSize);
...
MatrixServerDll_API void OnInitInstrumentADCDataFrame(m_oADCDataFrameStruct* pADCDataFrame,
	m_oInstrumentCommInfoStruct* pCommInfo, m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pADCDataFrame == NULL) || (pCommInfo == NULL))
	{
...
MatrixServerDll_API void OnInitInstrumentList(m_oInstrumentListStruct* pInstrumentList,
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pInstrumentList == NULL))
	{
...
MatrixServerDll_API void OnInitRoutList(m_oRoutListStruct* pRoutList,
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pRoutList == NULL))
	{
...
MatrixServerDll_API void OnInitADCDataBufArray(m_oADCDataBufArrayStruct* pADCDataBufArray,
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pADCDataBufArray == NULL))
	{
...
MatrixServerDll_API void OnInitOptTaskArray(m_oOptTaskArrayStruct* pOptTaskArray,
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pOptTaskArray == NULL))
	{
...
MatrixServerDll_API void GenOneOptTask(unsigned int uiIndex, unsigned int uiStartFrame, 
	...
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pOptTaskArray == NULL) || (pInstrumentList == NULL))
	{
		...
		iter != pInstrumentList->m_oIPInstrumentMap.end(); iter++)
	{
		if (iter->second->m_iInstrumentType == pConstVar->m_iInstrumentTypeFDU)
		{
			...
			oOptInstrument.m_iLineNb = iter->second->m_iLineNb;
...
MatrixServerDll_API bool ParseInstrumentHeadFrame(m_oHeadFrameStruct* pHeadFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pHeadFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pHeadFrame->m_pCommandStruct, 
		pHeadFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pHeadFrame->m_oSecHeadFrame);
	return bReturn;
}
// 解析尾包帧
MatrixServerDll_API bool ParseInstrumentTailFrame(m_oTailFrameStruct* pTailFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pTailFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pTailFrame->m_pCommandStruct, 
		pTailFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pTailFrame->m_oSecTailFrame);
	return bReturn;
}
// 解析IP地址设置应答帧
MatrixServerDll_API bool ParseInstrumentIPSetReturnFrame(m_oIPSetFrameStruct* pIPSetFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pIPSetFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pIPSetFrame->m_pCommandStructReturn, 
		pIPSetFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pIPSetFrame->m_oSecIPSetFrame);
	return bReturn;
}
// 解析尾包时刻查询帧
MatrixServerDll_API bool ParseInstrumentTailTimeReturnFrame(m_oTailTimeFrameStruct* pTailTimeFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pTailTimeFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pTailTimeFrame->m_pCommandStructReturn, 
		pTailTimeFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pTailTimeFrame->m_oSecTailTimeFrame);
	return bReturn;
}
// 解析时统设置应答帧
MatrixServerDll_API bool ParseInstrumentTimeDelayReturnFrame(m_oTimeDelayFrameStruct* pTimeDelayFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pTimeDelayFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pTimeDelayFrame->m_pCommandStructReturn, 
		pTimeDelayFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pTimeDelayFrame->m_oSecTimeDelayFrame);
	return bReturn;
}
// 解析ADC参数设置应答帧
MatrixServerDll_API bool ParseInstrumentADCSetReturnFrame(m_oADCSetFrameStruct* pADCSetFrame,
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pADCSetFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pADCSetFrame->m_pCommandStructReturn, 
		pADCSetFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pADCSetFrame->m_oSecADCSetFrame);
	return bReturn;
}
// 解析误码查询应答帧
MatrixServerDll_API bool ParseInstrumentErrorCodeReturnFrame(m_oErrorCodeFrameStruct* pErrorCodeFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pErrorCodeFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pErrorCodeFrame->m_pCommandStructReturn,
		pErrorCodeFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pErrorCodeFrame->m_oSecErrorCodeFrame);
	return bReturn;
}
// 解析ADC数据接收帧
MatrixServerDll_API bool ParseInstrumentADCDataRecFrame(m_oADCDataFrameStruct* pADCDataFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pADCDataFrame == NULL))
	{
	...
	bReturn = ParseInstrumentFrame(pADCDataFrame->m_pCommandStructReturn, 
		pADCDataFrame->m_pcRcvFrameData, pConstVar);
	LeaveCriticalSection(&pADCDataFrame->m_oSecADCDataFrame);
	return bReturn;
}
// 生成心跳帧
MatrixServerDll_API void MakeInstrumentHeartBeatFrame(m_oHeartBeatFrameStruct* pHeartBeatFrame, 
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pHeartBeatFrame == NULL))
	{
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pHeartBeatFrame->m_oSecHeartBeat);
	SendFrame(pHeartBeatFrame->m_oHeartBeatSocket, pHeartBeatFrame->m_pcSndFrameData, 
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pIPSetFrame->m_oSecIPSetFrame);
	SendFrame(pIPSetFrame->m_oIPSetFrameSocket, pIPSetFrame->m_pcSndFrameData, 
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pTailTimeFrame->m_oSecTailTimeFrame);
	SendFrame(pTailTimeFrame->m_oTailTimeFrameSocket, pTailTimeFrame->m_pcSndFrameData, 
...
MatrixServerDll_API void MakeInstrumentDelayTimeFrame(m_oTimeDelayFrameStruct* pTimeDelayFrame, 
	m_oConstVarStruct* pConstVar, m_oInstrumentStruct* pInstrument)
{
	if ((pConstVar == NULL) || (pTimeDelayFrame == NULL) || (pInstrument == NULL))
	{
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pTimeDelayFrame->m_oSecTimeDelayFrame);
	SendFrame(pTimeDelayFrame->m_oTimeDelayFrameSocket, pTimeDelayFrame->m_pcSndFrameData, 
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pADCSetFrame->m_oSecADCSetFrame);
	SendFrame(pADCSetFrame->m_oADCSetFrameSocket, pADCSetFrame->m_pcSndFrameData, 
	...
	m_oConstVarStruct* pConstVar, unsigned int uiIP, unsigned short usDataPoint)
{
	...
	MakeInstrumentFrame(pADCDataFrame->m_pCommandStructSet, pConstVar, pADCDataFrame->m_pcSndFrameData);
	LeaveCriticalSection(&pADCDataFrame->m_oSecADCDataFrame);
}
// 发送ADC数据查询帧
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pADCDataFrame->m_oSecADCDataFrame);
	SendFrame(pADCDataFrame->m_oADCDataFrameSocket, pADCDataFrame->m_pcSndFrameData, 
	...
	m_oConstVarStruct* pConstVar, unsigned int uiBroadCastPort)
{
	...
	MakeInstrumentFrame(pErrorCodeFrame->m_pCommandStructSet, pConstVar, 
		pErrorCodeFrame->m_pcSndFrameData, pErrorCodeFrame->m_pcCommandWord, 
	...
	m_oConstVarStruct* pConstVar)
{
	...
	EnterCriticalSection(&pErrorCodeFrame->m_oSecErrorCodeFrame);
	SendFrame(pErrorCodeFrame->m_oErrorCodeFrameSocket, pErrorCodeFrame->m_pcSndFrameData, 
...
{
	if ((pConstVar == NULL) || (pInstrument == NULL))
	{
...
{
	if ((pConstVar == NULL) || (pInstrument == NULL) || (pRout == NULL))
	{
				...
				else
				{
					...
					if (bLocation == false)
					{
						...
						if (pInstrumentTemp1 == pInstrument)
						{
							...
							if (pInstrument->m_iRoutDirection == pConstVar->m_iDirectionTop)
							{
								pInstrument->m_iLineNb = pInstrumentTemp2->m_iLineNb - 1;
							}
							// 下方
							else if (pInstrument->m_iRoutDirection == pConstVar->m_iDirectionDown)
							{
...
{
	if ((pConstVar == NULL) || (pInstrument == NULL) || (pRoutList == NULL))
	{
...
MatrixServerDll_API void ProcHeadFrameOne(m_oHeadFrameThreadStruct* pHeadFrameThread)
{
		...
		&pHeadFrameThread->m_pInstrumentList->m_oSNInstrumentMap))
	{
		...
		else
		{
			AddMsgToLogOutPutList(pHeadFrameThread->m_pThread->m_pLogOutPut, "ProcHeadFrameOne", 
				OutPutErrorFrameInfo(pHeadFrameThread->m_pHeadFrame->m_pcRcvFrameData, 
				pHeadFrameThread->m_pHeadFrame->m_uiRcvBufferSize), 
	...
	str.Format(_T("接收到SN = 0x%x的仪器首包帧，首包时刻 = 0x%x，路由IP = 0x%x, 测线号 = %d，测点序号 = %d"), 
		...
		pInstrument->m_iLineNb, pInstrument->m_iPointIndex);
...
MatrixServerDll_API void  ProcIPSetReturnFrameOne(m_oIPSetFrameThreadStruct* pIPSetFrameThread)
{
	...
	{
		AddMsgToLogOutPutList(pIPSetFrameThread->m_pThread->m_pLogOutPut, "ProcIPSetReturnFrameOne", 
			OutPutErrorFrameInfo(pIPSetFrameThread->m_pIPSetFrame->m_pcRcvFrameData,
			pIPSetFrameThread->m_pIPSetFrame->m_uiRcvBufferSize),
...
MatrixServerDll_API void FreeInstrumentFromMap(m_oInstrumentStruct* pInstrument, 
	...
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pInstrumentList == NULL) || (pRoutList == NULL) || (pInstrument == NULL))
	{
...
MatrixServerDll_API void DeleteInstrumentAlongRout(m_oInstrumentStruct* pInstrument, 
	...
	m_oConstVarStruct* pConstVar)
{
	if ((pConstVar == NULL) || (pRout == NULL) || (pInstrument == NULL)
...
MatrixServerDll_API void ProcTailFrameOne(m_oTailFrameThreadStruct* pTailFrameThread)
{
		...
		&pTailFrameThread->m_pInstrumentList->m_oSNInstrumentMap))
	{
				...
				if (pRout->m_pTail != NULL)
				{
					...
					while(true)
					{
						...
						AddMsgToLogOutPutList(pTailFrameThread->m_pThread->m_pLogOutPut, 
							"ProcTailFrameOne", ConvertCStrToStr(str));
						// 将过期路由回收到空闲路由队列
		...
		{
			AddMsgToLogOutPutList(pTailFrameThread->m_pThread->m_pLogOutPut, "ProcTailFrameOne",
				OutPutErrorFrameInfo(pTailFrameThread->m_pTailFrame->m_pcRcvFrameData,
				pTailFrameThread->m_pTailFrame->m_uiRcvBufferSize), 
				ErrorType, IDS_ERR_ROUT_NOTEXIT);
		}
	}
	else
...
{
	if ((pConstVar == NULL) || (pRoutList == NULL) || (pConstVar == NULL))
	{
	...
	do 
	{
		...
		if (bSample == true)
		{
			if (pInstrumentNext->m_iTimeSetReturnCount == 0)
			{
				...
				AddMsgToLogOutPutList(pTimeDelayThread->m_pThread->m_pLogOutPut, 
					"CheckTimeDelayReturnByRout", ConvertCStrToStr(str), WarningType);
				return false;
			}
		}
		else
		{
			...
			if (pInstrumentNext->m_bTimeSetOK == false)
			{
				...
				AddMsgToLogOutPutList(pTimeDelayThread->m_pThread->m_pLogOutPut, 
					"CheckTimeDelayReturnByRout", ConvertCStrToStr(str));
				return false;
			}
		}
	} while (pInstrumentNext != pRout->m_pTail);
	...
	m_oTimeDelayThreadStruct* pTimeDelayThread)
{
	...
	else
	{
		AddMsgToLogOutPutList(pTimeDelayThread->m_pThread->m_pLogOutPut, "ProcTailTimeReturnFrameOne",
			OutPutErrorFrameInfo(pTimeDelayThread->m_pTailTimeFrame->m_pcRcvFrameData,
			pTimeDelayThread->m_pTailTimeFrame->m_uiRcvBufferSize),
...
MatrixServerDll_API void ProcTimeDelayReturnFrameOne(m_oTimeDelayThreadStruct* pTimeDelayThread)
{
	...
	else
	{
		AddMsgToLogOutPutList(pTimeDelayThread->m_pThread->m_pLogOutPut, "ProcTimeDelayReturnFrameOne",
			OutPutErrorFrameInfo(pTimeDelayThread->m_pTimeDelayFrame->m_pcRcvFrameData,
			pTimeDelayThread->m_pTimeDelayFrame->m_uiRcvBufferSize),
			ErrorType, IDS_ERR_IPMAP_NOTEXIT);
	}
}
// 处理时统设置应答
...
MatrixServerDll_API void ProcADCSetReturnFrameOne(m_oADCSetThreadStruct* pADCSetThread)
{
	...
	else
	{
		AddMsgToLogOutPutList(pADCSetThread->m_pThread->m_pLogOutPut, "ProcADCSetReturnFrameOne",
			OutPutErrorFrameInfo(pADCSetThread->m_pADCSetFrame->m_pcRcvFrameData,
			pADCSetThread->m_pADCSetFrame->m_uiRcvBufferSize),
			ErrorType, IDS_ERR_IPMAP_NOTEXIT);
	}
}
// 输出接收和发送帧的统计结果
...
MatrixServerDll_API void ProcErrorCodeReturnFrameOne(m_oErrorCodeThreadStruct* pErrorCodeThread)
{
	...
	else
	{
		AddMsgToLogOutPutList(pErrorCodeThread->m_pThread->m_pLogOutPut, "ProcErrorCodeReturnFrameOne",
			OutPutErrorFrameInfo(pErrorCodeThread->m_pErrorCodeFrame->m_pcRcvFrameData,
			pErrorCodeThread->m_pErrorCodeFrame->m_uiRcvBufferSize),
			ErrorType, IDS_ERR_IPSETMAP_NOTEXIT);
	}
}
// 处理误码查询应答帧
...
MatrixServerDll_API void ProcADCDataRecFrameOne(m_oADCDataRecThreadStruct* pADCDataRecThread)
{
	...
	if (TRUE == IfIndexExistInMap(uiIPInstrument, &pADCDataRecThread->m_pInstrumentList->m_oIPInstrumentMap))
	{
		...
		if (pInstrument->m_uiADCDataActualRecFrameNum > 0)
		{
			...
			{
				AddMsgToLogOutPutList(pADCDataRecThread->m_pThread->m_pLogOutPut, "ProcADCDataRecFrameOne", 
					OutPutErrorFrameInfo(pADCDataRecThread->m_pADCDataFrame->m_pcRcvFrameData, 
					pADCDataRecThread->m_pADCDataFrame->m_uiRcvBufferSize), 
					ErrorType, IDS_ERR_FRAMEPOINT_LIMITOVER);
			}
...
{
	if ((pConstVar == NULL) || (pOptTask == NULL) || (pADCDataBuf == NULL))
	{
...
MatrixServerDll_API void OnOutPutResult(m_oEnvironmentStruct* pEnv)
{
		...
		iter != pEnv->m_pInstrumentList->m_oIPInstrumentMap.end(); iter++)
	{
		...
		AddMsgToLogOutPutList(pEnv->m_pLogOutPutErrorCode, "", ConvertCStrToStr(strOut));
...
MatrixServerDll_API void OnInit(m_oEnvironmentStruct* pEnv)
{
	...
	pEnv->m_pLogOutPutOpt->m_SaveLogFilePath = ConvertCStrToStr(strPath + SysOptLogFolderPath);
	...
	pEnv->m_pLogOutPutTimeDelay->m_SaveLogFilePath = ConvertCStrToStr(strPath + TimeDelayLogFolderPath);
	...
	pEnv->m_pLogOutPutErrorCode->m_SaveLogFilePath = ConvertCStrToStr(strPath + ErrorCodeLogFolderPath);
