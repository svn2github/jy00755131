...
{
	for (int i=0; i<UDPPortNb; i++)
	{
		m_pUDPSocket[i]->Close();
		if (m_pUDPSocket[i] != NULL)
		{
			m_pUDPSocket[i] = NULL;
			delete m_pUDPSocket[i];
		}
	}
	if (m_uiPortNb != NULL)
	{
...
BOOL CUartToUdpDlg::OnInitDialog()
{
	...
	for (int i=0; i<UDPPortNb; i++)
	{	
		m_pUDPSocket[i] = new CUDPSocket(this);
		m_pUDPSocket[i]->Create(m_uiPortNb[i],SOCK_DGRAM);
		m_pUDPSocket[i]->SetSockOpt(NULL,m_pUDPSocket[i],65536,SOL_SOCKET);
	}
...
void CUartToUdpDlg::OnCommMscomm1()
{
	...
	if(m_ctrlMSComm1.get_CommEvent()==2)
	{
		...
		if(len > 0) 
		{
			...
			for (int i=0; i<len; i++)
			{
				uart_buf[uart_count][i] = Rxdata[i];
			}
			OnSendFromUDP(uart_buf[uart_count], len);
			uart_count += 1;
...
void CUartToUdpDlg::OnSendFromUDP(unsigned char* TXDATA, unsigned int uiCount)
{
	...
	for (unsigned int i=0; i<uiFrameCount; i++)
	{
		...
		for (int j=0; j<UDPPortNb; j++)
		{
			...
			{
				m_pUDPSocket[j]->SendTo(ucTxdata, 256, uiPort, _T("127.0.0.1"));
			}
		}
	}
