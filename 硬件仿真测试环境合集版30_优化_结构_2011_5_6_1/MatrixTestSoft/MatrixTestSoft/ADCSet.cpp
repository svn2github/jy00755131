...
//************************************
int CADCSet::ADCSetFrameHead(unsigned int uiIPAim, bool bBroadCast, unsigned short	usCommand, unsigned short usPortAim)
{
...
{
	int iPos = 0;
	unsigned int uiIPAim = BroadCastPort;
	iPos = ADCSetFrameHead(uiIPAim, true, SendSetCmd, ADSetReturnPort);
	...
	iPos = ADCSetFrameHead(uiIPAim, true, SendQueryCmd, ADSetReturnPort);
...
// Parameter: unsigned int tnow
//************************************
void CADCSet::OnADCStartSample(unsigned int uiIPAim, unsigned int tnow)
{
		...
		if(NULL != pInstrument)	
		{
			if (pInstrument->m_bIPSetOK == true)
			{
				for (int i=0; i<InstrumentNum; i++)
				{
					...
					{
						iPos = ADCSetFrameHead(pInstrument->m_uiIPAddress, false, SendSetCmd, ADSetReturnPort);
	...
	for (int i=0; i<InstrumentNum; i++)
	{
		if (m_pSelectObject[i] == 1)
		{
			...
			iPos = ADCSetFrameHead(uiIPAim, false, SendSetCmd, ADSetReturnPort);
			OnADCReadContinuous(iPos);
			SendTo(m_ucFrameData, SndFrameSize, m_uiSendPort, IPBroadcastAddr);
