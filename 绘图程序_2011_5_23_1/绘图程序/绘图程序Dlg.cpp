...
void C绘图程序Dlg::drawChart(CChartViewer *viewer)
{
	...
	GetDlgItem(IDC_ChartViewer)->GetClientRect(&rectClient);
	GetDlgItem(IDC_HScrollBar)->MoveWindow(rectWindow.left, rectWindow.bottom, rectWindow.Width(), SetHScrollWidth);
	GetDlgItem(IDC_VScrollBar)->MoveWindow(rectWindow.right, rectWindow.top, SetVScrollWidth, rectWindow.Height());

	XYChart *c = new XYChart(rectClient.right, rectClient.bottom, 0xf0f0ff, 0, 1);
	...
	int iWidth = 0;
	int iHeight = 0;
	iWidth = rectClient.right - SetLegendWidth - m_uiIntervalNum * SetPlotAreaXInterval - SetLegendXInterval;
	iHeight = rectClient.bottom - SetPlotAreaYInterval - SetPlotAreaBottomInterval;
	...
	c->addLegend(rectClient.right - SetLegendWidth, SetLegendYInterval, false, "arialbd.ttf", 9)->setBackground(Chart::Transparent);
