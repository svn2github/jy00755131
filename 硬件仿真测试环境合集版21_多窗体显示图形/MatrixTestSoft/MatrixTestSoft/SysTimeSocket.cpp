		...
		if(NULL != pInstrument)	
		{
			if (pInstrument->m_bIPSetOK == true)
			{
				for (int i=0; i<GraphViewNum; i++)
				{
					...
					{
						m_pADCSet->ADCSetFrameHead(pInstrument->m_uiIPAddress, false, SendSetCmd, ADSetReturnPort);
...
//  	SendTo(m_pADCSet->m_cFrameData, SndFrameSize, m_uiSendPort, IPBroadcastAddr);
	...
	for (int i=0; i<GraphViewNum; i++)
	{
		...
		{
			uiIPAim	= 81 + 10*i;
			m_pADCSet->ADCSetFrameHead(uiIPAim, false, SendSetCmd, ADSetReturnPort);
...
// 	SendTo(m_pADCSet->m_ucFrameData, SndFrameSize, m_uiSendPort, IPBroadcastAddr);
	...
	m_pADCSet->OnSetTB(tnow, false);
	SendTo(m_pADCSet->m_ucFrameData, SndFrameSize, m_uiSendPort, IPBroadcastAddr);
	SetTimer(m_pwnd->m_hWnd, 1, 3000, NULL);
}
