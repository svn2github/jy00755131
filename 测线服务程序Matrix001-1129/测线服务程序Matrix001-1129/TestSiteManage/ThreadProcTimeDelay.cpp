...
void CThreadProcTimeDelay::ProcTimeDelay()
{
	...
	if(ProcTailTimeQuery()==false){
		CString strDesc;
		strDesc.Format("ProcTailTimeQuery False RoutIP=%d", m_pRout->m_uiRoutIP);
		// 设置运行状态数据
		m_pSiteData->m_oRunTimeDataList.Set(1, "CThreadProcTimeDelay", "ProcTimeDelay 1 find wait fail", strDesc);
		TRACE1("ProcTailTimeQuery False RoutIP=%d\r\n", m_pRout->m_uiRoutIP);
		return;
	}
	// 判断：不能继续处理
	if(false == JudgeProcCondition())
	...
	if(Calc_skip!=0){
		CString strDesc;
		strDesc.Format("ProcTimeDelay 2 find wait fail RoutIP=%d", m_pRout->m_uiRoutIP);
		// 设置运行状态数据
	...
	if(ProcTimeDelayCalculate_sys()==false){
		CString strDesc;
		strDesc.Format("ProcTimeDelay 3 find wait fail RoutIP=%d", m_pRout->m_uiRoutIP);
		// 设置运行状态数据
...
void CThreadProcTimeDelay::GetInstrumentForTailTimeQuery()
{
	...
	while(true)
	{
		...
		pInstrument->m_uiTailFrameCount=pInstrument->m_uiTailFrameCount|0x80000000;
		// 得到下一仪器
...
CInstrument* CThreadProcTimeDelay::GetNextInstrument(CInstrument* pInstrument)
{
	...
	{
		if(TRUE != m_pSiteData->m_oSNInstrumentMap.GetInstrument(pInstrumentNext->m_uiSN, pInstrumentNext))
			return NULL;
		// 下一个仪器没有设置IP地址
		...
		if(iFrameCount > 0)	// 有尾包时刻查询应答帧
		{
			...
			for(int i = 0; i < iFrameCount; i++)	// 依次得到尾包时刻查询应答帧
			{
				...
				if(TRUE == GetInstrumentByIP(m_pSocketTailTimeFrame->m_oFrameTailTime.m_uiIPSource, pInstrument))
				{
					...
					pInstrument->m_uiTailFrameCount=pInstrument->m_uiTailFrameCount&0x7fffffff;
/*
...
*/
					...
					CString strDesc;
					strDesc.Format("时延应答帧错误- IP=%d",m_pSocketTailTimeFrame->m_oFrameTailTime.m_uiIPSource); 
...
bool CThreadProcTimeDelay::ProcTailTimeQueryOnce(int iTimeDelay)
{
	...
	if(m_pSiteData->OnMarkerCount<1)
		return false;
	// 向仪器发送尾包时刻查询帧
	if(iTimeDelay!=151)
		SendTailTimeFrame();
...
bool CThreadProcTimeDelay::ProcTimeDelayCalculate_sys()
{
	...
	CInstrument* pInstrument = NULL;
	CInstrument* pInstrumentNext = NULL;
	CInstrument* pInstrument_temp=NULL;
	pInstrument = m_pRout->m_pHead;
	...
	{	 
		// 得到下一仪器
		pInstrumentNext = GetNextInstrument(pInstrument);
		if(pInstrumentNext == m_pRout->m_pTail ||  pInstrumentNext==NULL)
		{
			break;
		}
		if((False == pInstrument->m_bTailTimeQueryOK) || pInstrument->m_bIPSetOK==false ||  pInstrument==NULL)
		{
			break;
		}
	...
	int iTimeDelayL=0;
	int inDelaylast=0,Delayinit0=8569,DelayPower=2300;	//4112 8565=14个站
	int idiff=17;//头尾补偿
	int iDelayLdiff=0;
	...
	{	 
		...
		if(pInstrumentNext==NULL)
			break;
		pInstrument_temp=NULL;
		if(pInstrumentNext->m_uiInstrumentType==2){
			CString strDesc;
			pInstrument_temp = pInstrumentNext;
			pInstrumentNext = GetNextInstrument(pInstrument_temp);
			if(pInstrumentNext==NULL)
				break;
			//TRACE3("时延电源2 m_usReceiveTime=%d m_usSendTime=%d m_uiTimeLow=%d\r\n", pInstrument_temp->m_usReceiveTime,pInstrumentNext->m_usSendTime ,(pInstrument_temp->m_usReceiveTime-pInstrumentNext->m_usSendTime )&0xff);
			//strDesc.Format("Get3 Sn=%x,m_uiSystemTimeSite= %u m_uiTimeSystem= %u m_uiNetTime= %u m_uiTimeDelay= %d m_uiTimeHigh= %d m_uiTimeLow= %d rev= %u send= %u %d",pInstrument_temp->m_uiSN,m_uiSystemTimeSite,pInstrument_temp->m_uiTimeSystem,pInstrument_temp->m_uiNetTime,iTimeDelayHH,pInstrument_temp->m_uiTimeHigh,pInstrument_temp->m_uiTimeLow,pInstrument_temp->m_usReceiveTime,pInstrumentNext->m_usSendTime ,(pInstrument_temp->m_usReceiveTime,pInstrumentNext->m_usSendTime)); 
		...
		if(pInstrument->m_uiInstrumentType==1){
			...
			//TRACE3("ProcTimeDelaySet00 IP=%d TBH=%d  TBL=%d\r\n", pInstrument->m_uiIP,pInstrument->m_uiTimeHigh,pInstrument->m_uiTimeLow);
			pInstrument = pInstrumentNext;
			continue;
		}

		if(m_pSiteData->iDelayfrist==3){
			// 本地时间修正高位
	...
	//	iTimeDelayH =pInstrument->m_uiNetTime-pInstrument->m_uiTimeSystem;
		t1=pInstrument->m_usReceiveTime-pInstrumentNext->m_usSendTime;
		if(pInstrumentNext == m_pRout->m_pTail || itime==0)
			t1=t1+idiff;
		if(t1<0)
			t1=t1+TLOffset;
		inDelaylast=t1;
		...
		if(pInstrument_temp==NULL)
			iTimeDelayL=iTimeDelayL-(inDelaylast-Delayinit0);
		else
			if(pInstrument_temp->m_uiInstrumentType!=2 )
				iTimeDelayL=iTimeDelayL-(inDelaylast-Delayinit0);
			else
				iTimeDelayL=iTimeDelayL+37;
		//iTimeDelayL=0;
		pInstrumentNext->m_uiTimeLow =(iTimeDelayL+iDelayLdiff)%0x3fff;
		//TRACE3("时延SN=%d inDelaylast=%d m_uiTimeLow=%d\r\n", pInstrument->m_uiSN,inDelaylast&0xff,pInstrumentNext->m_uiTimeLow);
		//inDelaylast = inDelaylast&0x0ff;
		//iDelayL = iDelayL + inDelaylast/2-12 ;
		//pInstrumentNext->m_uiTimeLow =iDelayL %0x3fff;
		itime++;
		// 加入队列
		m_olsInstrument.AddTail(pInstrument);
		// 加入索引表
...
*/		if(m_pSiteData->iDelayfrist==2){
			...
			CString strDesc;
			strDesc.Format("Get1 Sn=%x,m_uiSystemTimeSite= %u m_uiTimeSystem= %u m_uiNetTime= %u m_uiTimeDelay= %d m_uiTimeHigh= %d m_uiTimeLow= %d rev= %u send= %u ",pInstrument->m_uiSN,m_uiSystemTimeSite,pInstrument->m_uiTimeSystem,pInstrument->m_uiNetTime,iTimeDelayHH,pInstrument->m_uiTimeHigh,pInstrument->m_uiTimeLow,pInstrument->m_usReceiveTime,pInstrumentNext->m_usSendTime ); 
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcTimeDelay", "Calculate1", strDesc);
		}
		pInstrument = pInstrumentNext;
		// 路由尾仪器
...
*/	// 加入队列
	...
	m_oInstrumentMap.SetAt(pInstrument->m_uiIP, pInstrument);
	CString strDesc;
	strDesc.Format("Get2 Sn=%x,m_uiSystemTimeSite= %u m_uiTimeSystem= %u m_uiNetTime=%u m_uiTimeDelay= %d m_uiTimeHigh= %u m_uiTimeLow= %u rev= %u %u data=%x order=%x",pInstrument->m_uiSN,m_pSiteData->m_uiSystemTimeSite,pInstrument->m_uiTimeSystem,pInstrument->m_uiNetTime,iTimeDelayH,pInstrument->m_uiTimeHigh,pInstrument->m_uiTimeLow,pInstrument->m_usReceiveTime,inDelaylast-Delayinit0,m_pSiteData->NetDataCount,m_pSiteData->NetOrderCount); 
	m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcTimeDelay", "Calculate1", strDesc);
	return true;
}
