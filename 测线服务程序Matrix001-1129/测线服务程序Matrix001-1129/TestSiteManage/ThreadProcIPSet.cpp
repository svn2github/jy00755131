...
void CThreadProcIPSet::SendIPSetFrame()
{
	...
	while(NULL != pos)
	{
		...
		{
			if(pInstrument->m_uiSN==0 ||  pInstrument->m_uiIP==0)
				continue;
			...
			if(pInstrument->m_bIPSetOK ==true)
				break;
			CString strDesc;
			strDesc.Format("IP设置 Sn=%d IP=%d", pInstrument->m_uiSN, pInstrument->m_uiIP);
		...
		for(int i = 0; i < iFrameCount; i++)	// 依次检查每个仪器IP设置应答帧
		{
			...
			if(TRUE == m_oSNInstrumentMap.Lookup(uiSN, pInstrument))
			{
				...
				if( pInstrument!=NULL){
					...
					{
						if((3 == pInstrument->m_uiRoutDirection) && (NULL != pInstrument->m_pInstrumentRight)){
							pInstrument1=pInstrument;
							while (true)
							{
								pInstrument1->m_pInstrumentRight->m_uiTailFrameCount=0;
								pInstrument1=pInstrument1->m_pInstrumentRight;
								if (NULL == pInstrument1->m_pInstrumentRight || pInstrument1->m_pInstrumentRight->m_uiInstrumentType==1)
									break;
							}
						}
						if((4 == pInstrument->m_uiRoutDirection) && (NULL != pInstrument->m_pInstrumentLeft))
							pInstrument1=pInstrument;
							while (true)
							{
								pInstrument1->m_pInstrumentLeft->m_uiTailFrameCount=0;
								pInstrument1=pInstrument1->m_pInstrumentLeft;
								if (NULL == pInstrument1->m_pInstrumentLeft || pInstrument1->m_pInstrumentLeft->m_uiInstrumentType==1)
									break;
							}
					}
					//当尾包变化时重新设置时统
					m_pSiteData->m_oCriticalSectionSystemTimeSite.Lock();
						...
						//TRACE("m_bProcTimeDelayON_stop iDelayfrist=0\r\n");
