...
void CTestDataBuffer::SetElementTestDataOfOneFrame(byte* lpData, unsigned int uiElementIndex, unsigned int uiFrameIndex)
{
	...
	// 保存数据
	//memcpy(&m_pArrayTestData[iSaveIndex], lpData+6, m_uiGroupDataByteSize);
	//+6去除前2个数据，由于74改72，会丢失后面2个数据
	memcpy(&m_pArrayTestData[iSaveIndex], lpData+6, m_uiGroupDataByteSize);
	for(int i=0;i<m_uiGroupDataByteSize/3;i++){
		uiData=0;
		memcpy(&uiData, &lpData[(i+2) * 3], 3);
		if(uiData>0x800000)
			uiData=(0xffffff-uiData);
		//sprintf_s(charTemp,"%d \t",uiData);
		//strOutput += charTemp;
	}
	//TRACE1("CTestDataBuffer=%s\r\n",strOutput);
}
...
{
	// 数据置0
	memset(m_pArrayTestDataOfOneElement, 0x00, FrameADCount*4);
	// 得到测试单元一帧测试数据
