	...
	case 1:	// 上
		{
			if(0 == pRout->m_pHead->m_uiRoutIPTop)
			{
				...
				TRACE1("上路由删 m_pHead->m_uiSN=%x\r\n",pRout->m_pHead->m_uiSN);
			}
			break;
		}
	case 2:	// 下
...
bool CThreadProcMonitorRout::DeleteRoutLinkInstrumentWhenRoutTimeExpired(CRout* pRout)
{
	...
	// 判断路由对象的路由时间是否过期
	if(15000 < (uiTimeNow - pRout->m_uiRoutTime))	// 过期5000
	//if(3000 < (uiTimeNow - pRout->m_uiRoutTime))	// 过期
	{
		...
		CString strDesc;
		if(pRout->m_uiRoutDirection==3){
			;
			strDesc.Format("路由时间过期删除左仪器 m_uiRoutIP=%x,m_uiRoutDirection=%d",pRout->m_pHead->m_pInstrumentLeft->m_uiSN,pRout->m_uiRoutDirection);
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcMonitorRout", "DeleteRoutLinkInstrumentWhenRoutTimeExpired", strDesc);
			TRACE1("路由时间过期删除左仪器 m_uiRoutIP=%x\r\n",pRout->m_pHead->m_pInstrumentLeft->m_uiSN);
		}
		if(pRout->m_uiRoutDirection==4)
		{
			;
			strDesc.Format("路由时间过期删除右仪器 m_uiRoutIP=%x,m_uiRoutDirection=%d",pRout->m_pHead->m_pInstrumentRight->m_uiSN,pRout->m_uiRoutDirection);
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcMonitorRout", "DeleteRoutLinkInstrumentWhenRoutTimeExpired", strDesc);
			TRACE1("路由时间过期删除右仪器 m_uiRoutIP=%x\r\n",pRout->m_pHead->m_pInstrumentRight->m_uiSN);
		}
	...
	case 1:	// 上
		{
			...
			strDesc.Format("删除路由链接的仪器 m_uiRoutIP=%x,m_uiRoutDirection=%d",pRout->m_pHead->m_pCrossTop->m_uiSN,pRout->m_uiRoutDirection);
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcMonitorRout", "DeleteRoutLinkInstrument", strDesc);
			bDelete = true;
			break;
		}
	case 2:	// 下
		{
			...
			strDesc.Format("删除路由链接的仪器 m_uiRoutIP=%x,m_uiRoutDirection=%d",pRout->m_pHead->m_pCrossDown->m_uiSN,pRout->m_uiRoutDirection);
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcMonitorRout", "DeleteRoutLinkInstrument", strDesc);
			bDelete = true;
			break;
		}
	case 3:	// 左
		{
			...
			strDesc.Format("删除路由链接的仪器 m_uiRoutIP=%x,m_uiRoutDirection=%d",pRout->m_pHead->m_pInstrumentLeft->m_uiSN,pRout->m_uiRoutDirection);
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcMonitorRout", "DeleteRoutLinkInstrument", strDesc);
			bDelete = true;
			break;
		}
	case 4:	// 右
		{
			...
			strDesc.Format("删除路由链接的仪器 m_uiRoutIP=%x,m_uiRoutDirection=%d",pRout->m_pHead->m_pInstrumentRight->m_uiSN,pRout->m_uiRoutDirection);
			m_pSiteData->m_oRunTimeDataList.Set(3, "CThreadProcMonitorRout", "DeleteRoutLinkInstrument", strDesc);
			bDelete = true;
			break;
		}
