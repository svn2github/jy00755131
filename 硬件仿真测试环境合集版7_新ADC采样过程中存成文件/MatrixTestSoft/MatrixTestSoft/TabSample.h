...
public:
	...
	// 保存ADC数据到文件，每个设备采样约30000个点
	void OnSaveADCDataToFile(int iMathValueIdNum, int iGraphViewNum, char** cppSelectObjectName, vector <double>* dSampleData, SYSTEMTIME systime);
	// 未收到尾包计数
	unsigned int m_uiTailRecCount;
	// 滚动条左移
	void OnMoveScrollBarToRight(unsigned int uiMovePos);
	// 滚动条右移
	void OnMoveScrollBarToLeft(unsigned int uiMovePos);
