...
void CThreadManage::OnInit(void)
{
	...
	{
		m_ADCDataSocket.m_uiADCDataFrameCount[i] = 0;
		m_ADCDataSocket.m_usDataPointPrevious[i] = 0;
	}
...
void CThreadManage::OnClose(void)
{
	...
	m_IPSetSocket.Close();
	m_ADCDataSocket.m_listADC.RemoveAll();
	for (int i=0; i<GraphViewNum; i++)
	{
		m_ADCDataSocket.m_dSampleTemp[i].clear();
		m_ADCDataSocket.m_dADCSave[i].clear();
	}
