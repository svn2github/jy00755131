...
class CSensorGraphView : public CScrollView
{
	DECLARE_DYNCREATE(CSensorGraphView)

protected:
	CSensorGraphView();           // 动态创建所使用的受保护的构造函数
	virtual ~CSensorGraphView();
...
private:
	...
	void draw_devices(CDC* pDC, int iGridX, int iGridY, int test_type, float sensor_limit);
