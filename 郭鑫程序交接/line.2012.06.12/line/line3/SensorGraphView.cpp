...
// 画测线标尺
void CSensorGraphView::draw_mark_line_and_point(CDC* pDC, int iGridX, int iGridY)
{
	// 画测线标尺
...
// 画测线标尺和测点标尺
void CSensorGraphView::draw_mark_line(CDC* pDC, int iGridY)
{
	CPen oPen;
	CPen* pOldPen;
...
// 画测点标尺
void CSensorGraphView::draw_mark_point(CDC* pDC, int iGridX)
{
	int iStep = 1;
...
//加入设备到视图中
void CSensorGraphView::draw_devices(CDC* pDC, int iGridX, int iGridY, int test_type, float sensor_limit)
{
	...
	for(pns_itr = pn_sn_map.begin(); pns_itr != pn_sn_map.end(); ++pns_itr){
		...
		if(pns_itr->second != 0){
			...
			if(helper->find_device(di)){
				check_value = helper->get_test_result_by_type(di, test_type, sensor_limit);
				switch(check_value){
				case 0:// 0：未测试
					cbitmap_ptr = get_bitmap_ptr(iGridX, 3, 1);
					break;
...
//画出每一个设备
void CSensorGraphView::draw_device_unit(CDC* pDC, int iGridX, int iGridY, int test_type, CBitmap* bitmap_ptr)
{
	CDC dc;
	BITMAP bm;
...
//	m_pSiteData->m_oGraphSensorList.DrawGrid(pDC, m_iPaperX, m_iPaperY, m_iGridX, m_iGridY);
	...
	draw_devices(pDC, grid_x_, grid_y_, pParentFrame->m_uiTestType, 0.0f);
}
...
* 显示设备信息在属性对话框
*/
void CSensorGraphView::show_device_info(CPoint& point)
{
	matrix_data::data_base_helper::device_info di;
	int line_num = -1,
		point_nb = -1;
...
void CSensorGraphView::OnGraphViewSensorSetAuxiliary()
{
		...
		// 响应仪器图形视图弹出菜单设置辅助道
		pMainFrame->OnGraphViewInstrumentSetAuxiliary(3, 1);
}
...
void CSensorGraphView::zoom_graph()
{
...
* @return void
*/
void CSensorGraphView::OnShowTestValueInFloatWindow(matrix_data::data_base_helper::const_device_info& di, CPoint pointClient)
{
	CSensorGraphChildFrame* pParentFrame = (CSensorGraphChildFrame*)GetParentFrame();
	CString strTestUnit = "";
...
* @return void
*/
void CSensorGraphView::OnShowProperty(matrix_data::data_base_helper::const_device_info& di)
{
	CSensorGraphChildFrame* pParentFrame = (CSensorGraphChildFrame*)GetParentFrame();
...
void CSensorGraphView::load_bitmaps()
{
...
CBitmap* CSensorGraphView::get_bitmap_ptr(int grid_x/*x方向大小*/, 
...
void CSensorGraphView::show_context_menu(CPoint& point, CPoint& client_point)
{
	int line_num = -1,
		point_nb = -1, 
		sn = 0x0;
...
根据当前逻辑坐标点得到行号和点号
*/
bool CSensorGraphView::get_line_and_point_nb_according_to_point(CPoint& point, int& line_idx, int& point_nb)
{
	bool is_ok = false;
	// 判断点是否在绘图区域内
...
bool CSensorGraphView::judge_if_point_in_paper(CPoint& point)
{
	// 得到图形单元尺寸
...
void CSensorGraphView::zoom_in_drag( const CRect& client_rect, const CRect& select_area, CPoint& point_scroll_pos)
{
...
bool CSensorGraphView::get_device_info_from_point(CPoint& point, matrix_data::data_base_helper::device_info& di)
{
	bool is_ok = false;
	int line_num = -1,
		point_nb = -1;
