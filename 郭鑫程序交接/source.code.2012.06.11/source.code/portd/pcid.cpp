...
{
	APPLICATION_ENVIRONMENT::DEVICE_INFO_LIST::iterator itr;
	DEVICE_INFO_PTR di_ptr = NULL;
	...
	if(__application_environment.device_info_list_){
		for(itr = __application_environment.device_info_list_->begin();	itr != __application_environment.device_info_list_->end(); ++itr){
			di_ptr = *itr;
			if(di_ptr != NULL) delete di_ptr;
		}
		delete __application_environment.device_info_list_;
		__application_environment.device_info_list_ = NULL;
	}

	if(__application_environment.virtual_ip_table_map_){
		delete __application_environment.virtual_ip_table_map_;
		__application_environment.virtual_ip_table_map_ = NULL;
	}
...
BOOL assign_ip_button_down()
{
	...
	result = DialogBoxParam(__application_environment.instance_,
							MAKEINTRESOURCE(IDD_IP_SN_DIALOG),
							__application_environment.main_window_,
							dialog_sn_ip_list_proc,	
							NULL);	
...
BOOL set_window_control(bool is_save)
{
	...
	if(is_save){
		...
		SendMessage(GET_WINDOW_HANDLE(IDC_EDIT_LISTEN_IP), WM_GETTEXT, (WPARAM)sizeof(__application_environment.listen_ip_), 
			(LPARAM)__application_environment.listen_ip_);
		...
		SendMessage(GET_WINDOW_HANDLE(IDC_EDIT_LISTEN_IP), WM_SETTEXT, NULL, (LPARAM)__application_environment.listen_ip_);
