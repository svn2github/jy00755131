...
// 查询采集站错误代码
void CADCSet::OnQueryErrorCodeFdu(void)
{
	int iPos = 37;
	unsigned int uitmp = 0;
	m_ucFrameData[iPos] = 0x0b;
	iPos++;
	memcpy(&m_ucFrameData[iPos], &uitmp, 4);
	iPos += 4;
	m_ucFrameData[iPos] = 0x00;
...
// 查询交叉站故障计数
void CADCSet::OnQueryErrorCodeLAUX(void)
{
	int iPos = 37;
	unsigned int uitmp = 0;
	m_ucFrameData[iPos] = 0x1d;
	iPos++;
	memcpy(&m_ucFrameData[iPos], &uitmp, 4);
	iPos += 4;
	m_ucFrameData[iPos] = 0x1e;
	iPos++;
	memcpy(&m_ucFrameData[iPos], &uitmp, 4);
	iPos += 4;
	m_ucFrameData[iPos] = 0x00;
