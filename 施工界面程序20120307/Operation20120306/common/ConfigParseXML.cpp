...
{		
	...
	{	
		...
		//得到索引值
		strKey = _T("Index");
		pFDUConfig->m_dwLineDataRateIndex = CXMLDOMTool::GetElementAttributeUnsignedInt(&oElement, strKey);
		...
		//得到索引值
		strKey = _T("Index");
		pFDUConfig->m_dwHighpassFrequencyIndex = CXMLDOMTool::GetElementAttributeUnsignedInt(&oElement, strKey);

		// 找到入口
		strKey = _T("TimeManagement");
		lpDispatch = pXMLDOMDocument->getElementsByTagName(strKey);
		oNodeList.AttachDispatch(lpDispatch);
		// 找到入口
		lpDispatch = oNodeList.get_item(0);
		...
		//得到索引值
		strKey = _T("Index");
		pFDUConfig->m_dwTimeManageIndex = CXMLDOMTool::GetElementAttributeUnsignedInt(&oElement, strKey);

		// 找到入口
		strKey = _T("FromGPSConnection");
		lpDispatch = pXMLDOMDocument->getElementsByTagName(strKey);
		oNodeList.AttachDispatch(lpDispatch);
		// 找到入口
		lpDispatch = oNodeList.get_item(0);
		...
		//得到索引值
		strKey = _T("Index");
		pFDUConfig->m_dwGPSConnIndex = CXMLDOMTool::GetElementAttributeUnsignedInt(&oElement, strKey);

		// 找到入口
		strKey = _T("FromSourceControlConnection");
		lpDispatch = pXMLDOMDocument->getElementsByTagName(strKey);
		oNodeList.AttachDispatch(lpDispatch);
		// 找到入口
		lpDispatch = oNodeList.get_item(0);
		...
		//得到索引值
		strKey = _T("Index");
		pFDUConfig->m_dwSourceConnIndex = CXMLDOMTool::GetElementAttributeUnsignedInt(&oElement, strKey);
...
bool  CConfigParseXML::WriteCrewConfig(LPCTSTR pstrXMLFilePath,CFDUConfig* pFDUConfig)
{
	...
	{		
		...
		oElement.AttachDispatch(lpDispatch);
		vtVal.vt = VT_UI4;
		vtVal.ulVal = pFDUConfig->m_dwLineDataRateIndex;
		oVariant = vtVal;
		oElement.setAttribute(_T("Index"), oVariant);

		// 找到High-passCornerFrequency设置  
		strKey =_T("High-passCornerFrequency");
		lpDispatch = oXMLDOMDocument.getElementsByTagName(strKey);
		oNodeList.AttachDispatch(lpDispatch);
		lpDispatch = oNodeList.get_item(0);
		oElement.AttachDispatch(lpDispatch);
		vtVal.vt = VT_UI4;
		vtVal.ulVal = pFDUConfig->m_dwHighpassFrequencyIndex;
		oVariant = vtVal;
		oElement.setAttribute(_T("Index"), oVariant);
		...
		oElement.AttachDispatch(lpDispatch);
		vtVal.vt = VT_UI4;
		vtVal.ulVal = pFDUConfig->m_dwTimeManageIndex;
		oVariant = vtVal;
		oElement.setAttribute(_T("Index"), oVariant);

		// 找到FromGPSConnection设置
		strKey =_T("FromGPSConnection");
		lpDispatch = oXMLDOMDocument.getElementsByTagName(strKey);
		oNodeList.AttachDispatch(lpDispatch);
		lpDispatch = oNodeList.get_item(0);
		oElement.AttachDispatch(lpDispatch);
		vtVal.vt = VT_UI4;
		vtVal.ulVal = pFDUConfig->m_dwGPSConnIndex;
		oVariant = vtVal;
		oElement.setAttribute(_T("Index"), oVariant);

		// 找到FromSourceControlConnection设置
		strKey =_T("FromSourceControlConnection");
		lpDispatch = oXMLDOMDocument.getElementsByTagName(strKey);
		oNodeList.AttachDispatch(lpDispatch);
		lpDispatch = oNodeList.get_item(0);
		oElement.AttachDispatch(lpDispatch);
		vtVal.vt = VT_UI4;
		vtVal.ulVal = pFDUConfig->m_dwSourceConnIndex;
		oVariant = vtVal;
		oElement.setAttribute(_T("Index"), oVariant);
