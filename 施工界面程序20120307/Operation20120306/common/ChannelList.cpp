...
: m_uiCountAll(FDU_MAXINUM)
{
	m_pArrayChannel = NULL;
}
...
{
	CString strFileName = CLIENTDIR_DATAFILE;
	strFileName +=_T("FileChannel.dat");
	CFile oFile;
	BOOL bData;
	...
	for(unsigned int i = 0; i < m_uiCountAll; i++)
	{
		...
		if(true == pChannelSetupData->m_bInUsed)
		{
			...
			*/
			// 此处与测线程序的中相同类的定义不同，修改为通过线号、点号和道号计算出IP地址
			// pChannelSetupData->m_uiIP = CChannelSetupData::CreateIP(pChannelSetupData->m_uiNbLine,pChannelSetupData->m_uiNbPoint,pChannelSetupData->m_uiNbChannel);
