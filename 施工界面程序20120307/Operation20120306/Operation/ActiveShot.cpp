 ...
 * @return 返回已发送的字节数。
 */
int   CActiveShot::SendShotSpreadChannel(CShotSpreads* m_AllSpreads,unsigned long m_dwSpreadSFL)
{
	...
	INT_PTR		nCount=0;
	INT_PTR		mCount = m_AllSpreads->GetCount(); 
	WORD	wLen; 
	...
	{
		CShotSpread *pSpread = m_AllSpreads->GetShotSpreadBySN(m_dwSpreadSFL+j);
...
//	{		
		...
		for(int j=0;j<mCount;j++)
		{
			...
			for(i=0;i<nIndex;i++)
			{
				...
				pChannel =&( pSpread->m_arrSpread[i+nCycleNum*1000] );
				...
				//TRACE3("SendSPChannel1 nIndex=%d,IP=%d %d\r\n",nIndex,pChannel->m_dwIP,nCount);
				}
			indexlen+=nIndex;
			// 采集道数量，不是字节数
...
/*
	...
	for(int j=0;j<mCount;j++)
	{
		...
		{
			pChannel =&( pSpread->m_arrSpread[i+nCycleNum*1000] );
			memcpy_s(pBuf+21+21*(i+indexlen),4,&(pChannel->m_dwIP),4);
			memcpy_s(pBuf+25+21*(i+indexlen),4,&(pChannel->m_dwGain),4);
			memcpy_s(pBuf+29+21*(i+indexlen),4,&(pChannel->m_dwLineNb),4);
			memcpy_s(pBuf+33+21*(i+indexlen),4,&(pChannel->m_dwPointNb),4);
			memcpy_s(pBuf+37+21*(i+indexlen),4,&(pChannel->m_dwChannelNb),4);			
			pBuf[41+21*(i+indexlen)] = pChannel->m_byType;
			//TRACE3("SendSPChannel2 i=%d,IP=%d %d\r\n",i,pChannel->m_dwIP,nCount);
		}		
		indexlen+=nIndex;
		
	}
	// 帧校验
	pBuf[21+21*indexlen]=0;
	// 帧尾
	pBuf[21+21*indexlen+1] = 0x14;
	pBuf[21+21*indexlen+2] = 0x6F;
	nIndex = SendToServer(pBuf,24+21*indexlen);
*/
	int tmp=0;
	memcpy(&tmp,pBuf+9,2);
	//TRACE2("wFrmCmdCode=%x len=%d\r\n",tmp,24+21*indexlen);
	delete []pBuf;
	return nIndex;
