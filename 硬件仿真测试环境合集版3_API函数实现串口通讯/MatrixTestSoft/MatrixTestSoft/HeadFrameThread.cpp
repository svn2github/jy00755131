...
{
	m_hHeadFrameThreadClose = ::CreateEvent(false, false, NULL, NULL);	// 创建事件对象
	::ResetEvent(m_hHeadFrameThreadClose);	// 设置事件对象为无信号状态
}
// 线程函数在Run中执行
int CHeadFrameThread::Run()
{
	...
	::SetEvent(m_hHeadFrameThreadClose);	// 设置事件对象为有信号状态
	// 返回
