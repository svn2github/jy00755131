...
void CSiteData::OnInit()
{
	...
	// 建立客户端内部网络命令发送处理网络对象
	m_oSocketInterfaceCmdSend.OnInit(m_strIPForInterface, m_uiPortForCmdSend, m_uiPortForCmdSendTo);	
...
CString CSiteData::GetSensorTestTypeLimitStringByID(unsigned int uiTestType)
{
	...
	if(TRUE == m_oTestLimitListSensor.m_oTestLimitDataMap.Lookup(uiTestLimitNb, oTestLimitData))
	{
		...
		if(15 == uiTestType)
		{
			CString strTestLimit1;
			CString strTestLimit2;
			strTestLimit1 = strTestLimit;
			uiTestLimitNb = 1501;
			if(TRUE == m_oTestLimitListSensor.m_oTestLimitDataMap.Lookup(uiTestLimitNb, oTestLimitData))
			{
				strTestLimit2.Format("%3.2f%s", oTestLimitData.m_fLimit, oTestLimitData.m_strUnit);
				strTestLimit.Format("%s-%s", strTestLimit1, strTestLimit2);
			}
		}
		return strTestLimit;
	}
...
/*
	...
	while(true)
	{
		bData = pConnection->GetFile("Matrix.xml", "Matrix.xml", FALSE);
		if(TRUE == bData)
		{
			bReturn = true;
			break;
...
/*
	...
	while(true)
	{
		bData = pConnection->PutFile("Matrix.xml", "Matrix.xml");
		if(TRUE == bData)
		{
			bReturn = true;
			break;
...
/*
	...
	bData = GetDataFileFromFTPServer(pConnection, "FileInstrument.dat");
	if(TRUE == bData)
	{
		bData = GetDataFileFromFTPServer(pConnection, "FileRout.dat");
		if(TRUE == bData)
		{
			bData = GetDataFileFromFTPServer(pConnection, "FileChannel.dat");
			if(TRUE == bData)
			{
				bReturn = true;
			}
...
* @param CString strFile 文件名
* @return bool true：成功；false：失败
*/
bool CSiteData::GetDataFileFromFTPServer(CFtpConnection* pConnection, CString strFile)
{
	BOOL bData;
	int iCount = 0;
	while(true)
	{
		bData = pConnection->GetFile(strFile, strFile, FALSE);
		if(TRUE == bData)
		{
			return true;
		}
...
void CSiteData::LoadPortSetupData(void)
{
	...
	strKey = "PortForCmdSendTo";
	m_uiPortForCmdSendTo = CXMLDOMTool::GetElementAttributeUnsignedInt(&oElement, strKey);
